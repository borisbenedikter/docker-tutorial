...
To save the output files generated by the application, you can use the `-v` flag to mount a volume from the host machine to the container.

```bash
docker run -v /path/to/host/dir:/path/to/container/dir random-matrix-generator:latest
```

All the files generated by the application in the `/path/to/container/dir` directory will be saved in the `/path/to/host/dir` directory on the host machine.
Note that the colon `:` is used to separate the host path from the container path.
